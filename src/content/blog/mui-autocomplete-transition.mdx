...
import LoadingSelectExample from "../../components/blog/mui-autocomplete-transition/examples/LoadingSelectExample.tsx";
...
import FirstTransitionIsPendingExample from "../../components/blog/mui-autocomplete-transition/examples/FirstTransitionIsPendingExample.jsx";
import IsPendingConsoleExample from "../../components/blog/mui-autocomplete-transition/examples/IsPendingConsoleExample.jsx";
import CombinedIsPendingWithStateExample from "../../components/blog/mui-autocomplete-transition/examples/CombinedIsPendingWithStateExample.jsx";
import IsPendingNullExample from "../../components/blog/mui-autocomplete-transition/examples/IsPendingNullExample.jsx";

Working with Next.js and Relay over the past year, I have found myself using
React's Suspense and Transitions a lot more than I did previously. I had very
minimal experience with Suspense but none when it came to Transitions.

What is a Transition, and how do we use them in React? As explained in the
...
While Next and Relay both utilize Transitions, there were abstracted in such a
way that I never properly understood how they worked. Despite months of using
`<Suspense>`, `useTransition` (and `startTransition`), and `loading.tsx`, I
couldn't reliably explain how they worked. So I set out to learn Transitions,
and what I found has left me a little bit confused.

Let's dive in.

## Non Suspense Example

Here's an autocomplete for selecting a dessert.

<NonTransitionExample client:only="react" />

When you select a dessert that has flavor options ("Mom's Famous Chocolate Chip
Cookies" don't have any flavor options), another autocomplete is rendered below
it, prompting you to select a dessert flavor. The flavors are fetched from an
API, which means we want to show a loading state while that fetch is happening.
The `loading` state for `<FlavorSelect>` in the example below is determined by
the absence of `flavorOptions`.

```jsx
<FlavorSelect loading={!flavorOptions} />
```

Once the flavors have finished loading, `setFlavorOptions` is called with that
result, and the `loading` prop is set to true. There is no Suspense or
`startTransition`. Just local state variables that get called after the async
fetch is completed. Pretty standard stuff.

## Prevent Transitions From Triggering Parent Loading States
...
the app's Suspense fallback UI.
